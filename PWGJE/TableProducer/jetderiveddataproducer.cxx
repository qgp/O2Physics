...
struct JetDerivedDataProducerTask {
  ...
  {
    jetCollisionsTable(collision.posZ(), JetDerivedDataUtilities::setEventSelectionBit(collision));
  ...
  void fillJetParticlesTable(T const& collision, U const& particle)
  {
    ...
    jetMcParticlesTable(collision.globalIndex(), particle.pt(), particle.eta(), particle.phi(), particle.y(), particle.e(), particle.pdgCode(), particle.statusCode(), particle.isPhysicalPrimary(), mothersId, daughtersId);
  ...
  void processChargedMCD(soa::Join<aod::Collisions, aod::EvSels, aod::McCollisionLabels>::iterator const& collision,
                         soa::Filtered<soa::Join<aod::Tracks, aod::TrackSelection, aod::McTrackLabels>> const& tracks,
